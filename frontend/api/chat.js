...
- Para cada restaurante usa EXACTAMENTE este formato (respeta los saltos de línea):
...
IMPORTANTE: Deja siempre una línea en blanco ANTES de la lista con guiones y DESPUÉS del nombre del restaurante.
...
- Si el usuario dice "y por el centro?" después de preguntar por italianos, entiende que sigue buscando italianos
- Si dice "algo más barato", recuerda qué tipo de comida estaba buscando
- Mantén coherencia con lo que has recomendado antes`
...
export default async function handler(req) {
  ...
  try {
    ...
    const { mensaje, historial, restaurantes } = body
    ...
    // Si hay historial, añadirlo (limitado a últimos 10 mensajes para no exceder tokens)
    if (historial && historial.length > 0) {
      ...
      console.log('Historial añadido al prompt:', contents.length, 'mensajes')
    } else {
      // Sin historial, solo el mensaje actual con el contexto completo
...
USUARIO: ${mensaje}`
    ...
    // Si hay historial, usar systemInstruction para el contexto
    if (historial && historial.length > 0) {
