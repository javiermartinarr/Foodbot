    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <span
          ...
          style={{ backgroundColor: 'var(--badge-bg)', color: 'var(--card-title)' }}
        >
          {restaurante.puntuacion?.toFixed(1) || '-'}
        </span>
      </div>
      <p className="text-xs truncate" style={{ color: 'var(--card-subtitle)' }}>
        {restaurante.tipo_comida} · {restaurante.barrio}
      </p>
      {restaurante.plato_recomendado && (
        <p className="text-xs mt-2 truncate" style={{ color: 'var(--card-meta)' }}>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
      ...
      >
        ...
        <p className="text-sm mb-5" style={{ color: 'var(--card-subtitle)' }}>
        ...
        <button
          ...
          style={{ backgroundColor: '#D97706', boxShadow: '0 4px 14px rgba(217, 119, 6, 0.4)' }}
        >
          🎰 ¡Sorpréndeme!
        </button>
      </div>
          ...
          >
            ...
            >
              ...
              <span
                ...
                style={{ backgroundColor: 'var(--badge-bg)', color: 'var(--card-subtitle)' }}
              >
            ...
            .map((restaurante, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold truncate" style={{ color: 'var(--card-title)' }}>
                    {restaurante.nombre}
                  </h4>
                  <p className="text-sm truncate" style={{ color: 'var(--card-subtitle)' }}>
                    {restaurante.tipo_comida} · {restaurante.barrio}
                  </p>
                </div>
                <div
                  className="px-3 py-1.5 rounded-xl text-sm font-bold border"
                  style={{ backgroundColor: 'var(--badge-bg)', color: 'var(--card-title)', borderColor: 'var(--card-border)' }}
                >
                  {restaurante.puntuacion?.toFixed(1)}
                </div>
              </div>
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3 mt-6">
                <button
                  onClick={() => { setShowSorpresa(false); setSelectedRestaurante(restauranteSorpresa) }}
                ...
                <button
                  ...
                  style={{ backgroundColor: 'var(--filter-bg)', borderColor: 'var(--card-border)', color: 'var(--card-title)' }}
                >
                  🎲 Otro
                </button>
              </div>
      ...
      <style>{`
        ...
        .scrollbar-thin::-webkit-scrollbar-track { background: var(--badge-bg); border-radius: 3px; }
        .scrollbar-thin::-webkit-scrollbar-thumb { background: var(--card-border); border-radius: 3px; }
      `}</style>
    </div>
  )
}
